error id: file://<WORKSPACE>/src/main/java/com/wuxiansheng/shieldarch/marsdata/config/ApolloConfigService.java:_empty_/AppConfigService#
file://<WORKSPACE>/src/main/java/com/wuxiansheng/shieldarch/marsdata/config/ApolloConfigService.java
empty definition using pc, found symbol in pc: _empty_/AppConfigService#
empty definition using semanticdb
empty definition using fallback
non-local guesses:

offset: 747
uri: file://<WORKSPACE>/src/main/java/com/wuxiansheng/shieldarch/marsdata/config/ApolloConfigService.java
text:
```scala
 ...
 * Apollo配置服务（兼容层）
 * 
 * 为了保持向后兼容，保留 ApolloConfigService 类
 * 内部优先使用 NacosConfigService，如果 Nacos 不可用则回退到 Apollo
 * 
 * @deprecated 建议直接使用 AppConfigService 接口或 NacosConfigService
 */
@Slf4j
@Service
@Deprecated
public class ApolloConfigService implements AppCon@@figService {
    ...
    /**
     * 是否优先使用 Nacos（默认：true）
     */
    @Value("${config.use-nacos:true}")
    private boolean useNacos;
    
    /**
     * 配置命名空间常量（保持向后兼容）
     */
    public static final String OCR_LLM_CONF = AppConfigService.OCR_LLM_CONF;
    public static final String PRICE_FITTING_CONF = AppConfigService.PRICE_FITTING_CONF;
    public static final String QUALITY_MONITOR_CONF = AppConfigService.QUALITY_MONITOR_CONF;
    public static final String OCR_BUSINESS_CONF = AppConfigService.OCR_BUSINESS_CONF;
    
    /**
     * Apollo应用ID（保留用于兼容）
     */
    private static final String APOLLO_APP_ID = "llm-data-collect";
    
    /**
     * 获取配置（优先使用 Nacos，回退到 Apollo）
     * 
    ...
    public Map<String, String> getConfig(String namespace) {
        // 优先使用 Nacos
        if (useNacos && nacosConfigService != null) {
            try {
                Map<String, String> result = nacosConfigService.getConfig(namespace);
                if (result != null && !result.isEmpty()) {
                    log.debug("使用 Nacos 获取配置: namespace={}, count={}", namespace, result.size());
                    return result;
                }
            } catch (Exception e) {
                log.warn("Nacos 配置获取失败，回退到 Apollo: namespace={}, error={}", namespace, e.getMessage());
            }
        }
        
        // 回退到 Apollo
        return getConfigFromApollo(namespace);
    }
    
    /**
     * 从 Apollo 获取配置（原有实现）
     */
    private Map<String, String> getConfigFromApollo(String namespace) {
        ...
        try {
            ...
            log.debug("获取Apollo配置成功: namespace={}, count={}", namespace, result.size());
            
        } catch (Exception e) {
            log.error("获取Apollo配置失败: namespace={}, error={}", namespace, e.getMessage(), e);
            // 返回空Map，调用方需要处理空值情况
        }
    ...
    /**
     * 获取单个配置项的值（优先使用 Nacos，回退到 Apollo）
     */
    @Override
    public String getProperty(String namespace, String key, String defaultValue) {
        ...
        if (useNacos && nacosConfigService != null) {
            try {
                String value = nacosConfigService.getProperty(namespace, key, defaultValue);
                if (value != null) {
                    return value;
                }
            } catch (Exception e) {
                log.warn("Nacos 配置获取失败，回退到 Apollo: namespace={}, key={}, error={}", 
        ...
        try {
            ...
            log.warn("获取Apollo配置项失败: namespace={}, key={}, error={}", 
                namespace, key, e.getMessage());
            return defaultValue;
        }
    }
    ...
    /**
     * 检查配置项是否存在（优先使用 Nacos，回退到 Apollo）
     */
    @Override
    public boolean hasProperty(String namespace, String key) {
        ...
        if (useNacos && nacosConfigService != null) {
            try {
                return nacosConfigService.hasProperty(namespace, key);
            } catch (Exception e) {
                log.warn("Nacos 配置检查失败，回退到 Apollo: namespace={}, key={}, error={}", 
                    namespace, key, e.getMessage());
            }
        }
        ...
        try {
            ...
            log.warn("检查Apollo配置项失败: namespace={}, key={}, error={}", 
                namespace, key, e.getMessage());
            return false;
        }
    }
    ...
    public String getLLMPromptWithClassify(String businessName, String category, String defaultVal) {
        Map<String, String> params = getConfig(OCR_LLM_CONF);
        if (params.isEmpty()) {
            log.warn("invalid conf: {}, err: config is empty", OCR_LLM_CONF);
            return defaultVal;
        }
    ...
    public Map<String, String> getODs(String businessName) {
        ...
        try {
            ...
            return mapper.readValue(valStr, 
    ...
    public boolean isValidSupplier(String supplier, String businessName) {
        ...
        Map<String, String> params = getConfig(OCR_LLM_CONF);
        if (params.isEmpty()) {
            log.error("invalid conf: {}, err: config is empty", OCR_LLM_CONF);
        ...
        String valStr = params.get(key);
        if (valStr == null || valStr.isEmpty()) {
            log.error("empty supplier conf: {}, business_name: {}", OCR_LLM_CONF, businessName);
    ...
    public Map<Integer, String> getXLRuleIdToCityNameMap() {
        ...
        try {
            ...
            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            return mapper.readValue(valStr, 
                mapper.getTypeFactory().constructMapType(Map.class, Integer.class, String.class));
        } catch (Exception e) {
