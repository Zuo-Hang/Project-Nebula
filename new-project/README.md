...
│   │       ├── FrameExtractExecutor.java  # 对应StreamFrameExtractor
│   │       └── InferenceExecutor.java     # 对应AsyncInferenceWorker
│   ├── config/                # 配置类
│   ├── controller/             # HTTP接口
│   ├── service/                # 服务层
│   └── bootstrap/              # 启动类
...
│   ├── io/                     # IO操作（OcrClient, S3Client）
│   ├── executors/              # 执行器实现
│   └── pipeline/               # 处理流程
│
├── governance-core/            # 质量治理层（校验、自愈）
│   ├── validator/              # 校验器
│   │   ├── DualCheckValidator.java
│   │   └── rule/               # 业务规则插件目录
│   └── correction/             # 自愈逻辑
│       └── SelfCorrectionHandler.java
│
├── state-store/                # 持久层抽象与Redis实现
│   ├── interfaces/             # 状态存储接口
│   ├── redis/                  # Redis实现
│   │   ├── RedisStateStore.java
│   │   └── TaskSnapshot.java   # 状态快照实体
│   └── client/                 # 外部服务客户端
│       ├── LLMServiceClient.java
│       └── ComputerVisionServiceClient.java
│
...
```bash
# 启动所有依赖服务（Redis、RabbitMQ、MinIO、MySQL、RocketMQ、Prometheus、Grafana）
docker-compose up -d
